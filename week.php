...
echo "<td width=\"30%\"><u>".get_vocab("rooms")."</u><br>";
  ...
  if ($area_list_format == "select") {
	...
	{
		if ( $pview != 1 )
			echo "<h1><a href=\"week.php?year=$year&month=$month&day=$day&area=$area&room=$row[0]\" title=\"$row[2]\">";
		if ($row[0] == $room)
		{
			$this_room_name = htmlspecialchars($row[1]);
			if ( $pview != 1 )
				echo "$this_room_name</font></a></h1>\n";
		}
		else if ( $pview != 1 ) echo htmlspecialchars($row[1]) . "</a></h1>\n";
	}
} # end select if
...
# Show area and room:
$this_room_name = sql_query1("select room_name from $tbl_room where id=$room");
echo "<h2 align=center>$this_room_name</h2>\n";
...
#How far in advance is this slot
			$advance = mktime($hour, $minute, 0, $wmonth, $wday, $wyear) - mktime();
 		...
 		elseif ($descr != "")
 		{
			...
			echo " <a href=\"view_entry_fixed.php?id=$id"
